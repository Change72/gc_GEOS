...
/// Output operator
GEOS_DLL std::ostream& operator<< (std::ostream& os, const Envelope3d& o);
 ...
 *
 */
class GEOS_DLL Envelope3d {

public:

    friend std::ostream& operator<< (std::ostream& os, const Envelope3d& o);

    typedef std::unique_ptr<Envelope3d> Ptr;
     ...
     */
    Envelope3d(const CoordinateXYZ& p1, const CoordinateXYZ& p2)
    {
        init(p1, p2);
    }
     ...
     */
    explicit Envelope3d(const CoordinateXYZ& p)
    {
        init(p);
    }
     ...
     */
    explicit Envelope3d(const std::string& str);
     ...
     */
    static bool intersects(const CoordinateXYZ& p1, const CoordinateXYZ& p2,
    ...
    static bool intersects(
        const CoordinateXYZ& p1, const CoordinateXYZ& p2,
        const CoordinateXYZ& q1, const CoordinateXYZ& q2)
    {
        ...
        minq = std::min(q1.z, q2.z);
        maxq = std::max(q1.z, q2.z);
        minp = std::min(p1.z, p2.z);
        maxp = std::max(p1.z, p2.z);
        if(minp > maxq) {
            return false;
        }
        if(maxp < minq) {
     ...
     */
    bool intersects(const CoordinateXYZ& a, const CoordinateXYZ& b) const;
     ...
     * @param  p2  the second Coordinate
     */
    void init(const CoordinateXYZ& p1, const CoordinateXYZ& p2)
    {
     ...
     * @param  p  the Coordinate
     */
    void init(const CoordinateXYZ& p)
    {
     ...
     *
     * @return `max z - min z`, or 0 if this is a null Envelope
     */
    ...
    /** \brief
     * Returns the Envelope maximum z-value. `min z > max z` indicates
     * that this is a null Envelope.
     */
    ...
    /** \brief
     * Returns the Envelope minimum z-value. `min z > max z` indicates
     * that this is a null Envelope.
     */
     ...
     */
    bool centre(CoordinateXYZ& centre) const;
     ...
     */
    bool intersection(const Envelope3d& env, Envelope3d& result) const;
    ...
    /** \brief
     * Translates this envelope by given amounts in the X, Y and Z direction.
     *
     * @param transX the amount to translate along the X axis
     * @param transY the amount to translate along the Y axis
     ...
     * @param deltaX the distance to expand the envelope along the X axis
     * @param deltaY the distance to expand the envelope along the Y axis
     * @param deltaZ the distance to expand the envelope along the Z axis
     */
    ...
    {
        expandBy(p_distance, p_distance, p_distance);
    };
     ...
     */
    void expandToInclude(const CoordinateXYZ& p)
    {
    ...
    /** \brief
     * Enlarges the boundary of the Envelope so that it contains (x,y,z).
     *
     * Does nothing if (x,y,z) is already on or within the boundaries.
     *
     * @param  x  the value to lower the minimum x
     *            to or to raise the maximum x to
     * @param  y  the value to lower the minimum y
     *            to or to raise the maximum y to
     * @param  z  the value to lower the minimum z
     *            to or to raise the maximum z to
     */
     ...
     */
    void expandToInclude(const Envelope3d* other)
    {
        if(isNull()) {
            minx = other->minx;
            maxx = other->maxx;
     ...
     */
    bool
    contains(const Envelope3d& other) const
    {
        return covers(other);
    }
     ...
     */
    bool
    contains(const CoordinateXYZ& p) const
    {
     ...
     * @param y the y-coordinate of the point which this Envelope is being
     *          checked for containing
     * @param z the z-coordinate of the point which this Envelope is being
     *          checked for containing
     * @return `true` if `(x, y, z)` lies in the interior or on the boundary
     *         of this Envelope.
     */
    bool
    contains(double x, double y, double z) const
    {
        return covers(x, y, z);
    }
     ...
     */
    bool intersects(const CoordinateXYZ& other) const
    {
    ...
    /** \brief
     *  Check if the point (x, y, z) intersects (lies inside) the region of this Envelope.
     *
     * @param x the x-ordinate of the point
     * @param y the y-ordinate of the point
     ...
     * @return `true` if the point intersects this Envelope
     */
    bool intersects(double x, double y, double z) const
    {
     ...
     */
    bool intersects(const Envelope3d* other) const
    {
    ...
    */
    bool disjoint(const Envelope3d& other) const
    {
     ...
     * @param x the x-coordinate of the point which this Envelope is being checked for containing
     * @param y the y-coordinate of the point which this Envelope is being checked for containing
     * @param z the z-coordinate of the point which this Envelope is being checked for containing
     * @return `true` if `(x, y, z)` lies in the interior or on the boundary of this Envelope.
     */
    bool covers(double x, double y, double z) const {
        return x >= minx &&
               x <= maxx &&
               y >= miny &&
               y <= maxy &&
     ...
     */
    bool covers(const CoordinateXYZ* p) const
    {
        return covers(p->x, p->y, p->z);
    }
     ...
     */
    bool covers(const Envelope3d& other) const;

    bool
    covers(const Envelope3d* other) const
    {
        return covers(*other);
    }
     ...
     */
    bool equals(const Envelope3d* other) const;
     ...
     */
    double distance(const Envelope3d& env) const
    {
     ...
     */
    double maxDistance(const Envelope3d& other) const
    {
        CoordinateXYZ p(std::min(minx, other.minx), std::min(miny, other.miny), std::min(minz, other.minz));
        CoordinateXYZ q(std::max(maxx, other.maxx), std::max(maxy, other.maxy), std::max(maxz, other.maxz));
        return p.distance(q);
    }
     ...
     * distance is the Euclidean distance between the closest points.
     */
    double distanceSquared(const Envelope3d& env) const
    {
    ...
    static double distanceToCoordinate(
        const CoordinateXYZ& c,
        const CoordinateXYZ& p0,
        const CoordinateXYZ& p1)
    {
...
private:
    ...
    static double distanceVolume(double x0, double y0, double z0, double x1, double y1, double z1)
    {
        double dx = x1 - x0;
        double dy = y1 - y0;
        double dz = z1 - z0;
